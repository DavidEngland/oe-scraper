...
//        echo $zip2city[$zip].PHP_EOL;
        ...
        $content .= PHP_EOL . '<div itemprop="description">' . badger( $desc ) . "</div>\n";
        cwrap_it( 'excerpt:encoded', $desc );
        
        // Form bootstrap carousel from listing images.
        $slides = $xpath->query( '//*[@class="cyclephoto"]' );
        $content .= '<div id="' . $mls . '" class="carousel slide" data-ride="carousel">' . PHP_EOL;
        
        $content .= '  <ol class="carousel-indicators hidden-print">' . PHP_EOL;
        $ccontent = '  <div class="carousel-inner">' . PHP_EOL;
        ...
        foreach ( $links as $link ) {
            if ( 0 == $i ) {
                $content .= '    <li data-target="#' . $mls . '" data-slide-to="' . $i . '" class="active"></li>' . PHP_EOL;
            } //0 == $i
            else {
                $content .= '    <li data-target="#' . $mls . '" data-slide-to="' . $i . '"></li>' . PHP_EOL;
            }
            $class = ( 0 == $i ) ? 'item active' : 'item';
            $ccontent .= '    <div class="' . $class . '"><img src="' . $link . '" alt="' . $new_title . " image-" . $i . '" itemprop="image"  />' . PHP_EOL;
            $ccontent .= '    </div>' . PHP_EOL;
            $i++;
        } //$links as $link
        $content .= '  </ol>';
        $content .= PHP_EOL;
        $content .= $ccontent;
        $content .= '</div><!-- .carousel-inner -->' . PHP_EOL;
        $content .= '</div><!-- .carousel -->' . PHP_EOL;
